...
class Auth_lib
{
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						$this->error = array('ban' => $user->banned, 'banned' => $user->ban_reason);
					} else {                     
						...
						if ($user->activated == 0) {							// fail - not activated
    						...
    						$this->_set_session($user);                         //Add user data to session                
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation = false)
	{
	...
	function delete_user($user_id)
	{
		if (!is_null($user_id)) {
            ...
            // If user delete themself then delete session and logout
            if ($user_id == $this->ci->session_lib->get_user_id())
            {
                $this->logout();
            }
	...
	/**
	 * Clear send again user data session
	 *
	 * @return	void
	 */
    function reset_send_again()
    {
	 ...
	 * @return	void
	 */
	function _set_session($user_data)
	{
		// Set session data array
		$session_data = array(						
			'user_id'						    => $user_data->id,
			'username'						    => $user_data->username,
            'status'                            => ($user_data->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
			'log_in'					        => TRUE
		);
